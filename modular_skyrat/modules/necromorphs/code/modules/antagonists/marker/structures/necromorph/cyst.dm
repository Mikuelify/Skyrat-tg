...
/obj/structure/marker/special/cyst
	name = "cyst"
...
/obj/structure/marker/special/cyst/Initialize()
	.=..()
	if (!dummy)

		var/datum/proximity_trigger/cone/PT = new (holder = src, on_turf_entered = /obj/structure/marker/special/cyst/proc/nearby_movement, range = 5, extra_args = list(cone_angle, null))
		PT.register_turfs()
		set_extension(src, /datum/extension/proximity_manager, PT)

		addtimer(CALLBACK(src,/obj/structure/marker/special/cyst/proc/generate_payload),recharge_delay)
		generate_payload()

/obj/structure/marker/special/cyst/update_icon()
	if (payload)
		icon_state = "cyst-full"
	else
		icon_state = "cyst-empty"

/obj/structure/marker/special/cyst/get_blurb()
...
/obj/structure/marker/special/cyst/check_overlap(var/datum/click_handler/placement/P)
...
/obj/structure/marker/special/cyst/proc/generate_payload()
	payload = new(src)
	update_icon()

/obj/structure/marker/special/cyst/proc/nearby_movement(var/atom/movable/AM)
	if (!payload)
...
/obj/structure/marker/special/cyst/proc/fire(var/atom/target_atom)
	if (!payload)
		return	//Can't fire if we don't have a bomb ready
	...
	addtimer(CALLBACK(src,/obj/structure/marker/special/cyst/proc/generate_payload),recharge_delay)	//Start reloading the next one

//Being caught in its own blast is always a 1-hit kill
/obj/structure/marker/special/cyst/bioblast_act()
	qdel(src)
...
/datum/click_handler/placement/necromorph/cyst/placement_blocked(turf/candidate)
	...
	for(var/obj/structure/marker/special/cyst/C in candidate)
		if(C.dir == dir)
...
/datum/extension/mount/cyst/on_mount()
	.=..()
	var/obj/structure/marker/special/cyst/C = mountee
...
/datum/signal_ability/placement/corruption/cyst
	...
	id = "cyst"
	energy_cost = 60
	placement_atom = /obj/structure/marker/special/cyst
