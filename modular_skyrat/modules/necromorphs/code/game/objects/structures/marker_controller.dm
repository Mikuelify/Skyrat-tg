...
/datum/marker_controller/proc/SpawnExpansion()
	var/list/turfs = list()
	var/hatcheries_to_spawn = 2
	var/bulbs_to_spawn = 1
	var/spread_radius = 10
...
/datum/marker_controller/proc/SpawnStructureLoc(index, location)
	var/spawn_type
	switch(index)
		if(1)
			spawn_type = /obj/structure/necromorphs/marker/structure/bulb
		if(2)
			spawn_type = /obj/structure/necromorphs/marker/structure/spawner
...
/datum/marker_controller/Destroy()
	STOP_PROCESSING(SSobj, src)
	return ..()

/datum/marker_controller/proc/CoreRetaliated()
	structure_progression += PROGRESSION_RETALIATED
	active_resin.Cut()
	ActivateAdjacentResinRecursive(get_turf(our_core), 4)

/datum/marker_controller/proc/CoreDeath()
	active_resin.Cut()
	for(var/t in all_resin)
		var/obj/structure/necromorphs/marker/resin/a_resin = t
		a_resin.color = null
		if(a_resin.blooming)
	...
	for(var/t in other_structures)
		var/obj/structure/necromorphs/marker/structure/our_structure = t
		qdel(our_structure)
	return

/datum/marker_controller/proc/TrySpreadResin(obj/structure/necromorphs/marker/resin/spreaded_resin)
	. = RESIN_CANT_SPREAD
	var/turf/ownturf = get_turf(spreaded_resin)
	if(structure_progression > PROGRESSION_FOR_STRUCTURE)
		var/forbidden = FALSE
		for(var/obj/O in ownturf)
			if(istype(O, /obj/structure/necromorphs/marker/structure))
				forbidden = TRUE
				break
		if(!forbidden)
	...
	for(var/T in ownturf.GetAtmosAdjacentTurfs())
		...
		if(isspaceturf(T))
			if(!locate(/obj/structure/necromorphs/marker/structure/wall, ownturf))
				var/the_wall = new /obj/structure/necromorphs/marker/structure/wall(ownturf, blob_type)
	...
	for(var/T in possible_locs)
		...
		var/resinCount = 0
		var/placeCount = 1
		for(var/obj/structure/necromorphs/marker/resin/potato in iterated_turf)
			resinCount++
...
/datum/marker_controller/proc/SpawnResin(loc)
	...
	//Spawn the resin
	var/obj/structure/necromorphs/marker/resin/new_resin = new /obj/structure/necromorphs/marker/resin(loc, blob_type)
...
/datum/marker_controller/proc/ActivateAdjacentResinRecursive(turf/centrum_turf, iterations = 1)
	var/list/turfs = list()
	...
	for(var/t in turfs)
		var/turf/ite_turf = t
		for(var/obj/structure/necromorphs/marker/resin/potato in ite_turf)
			if(potato && potato.our_controller && potato.our_controller == src)
				active_resin[potato] = TRUE
				return

/datum/marker_controller/proc/ActivateAdjacentResin(turf/centrum_turf)
	if(!our_core)
...
/datum/marker_controller/process(delta_time)
	progress_to_spread++
	...
	if(!our_core)
		if(length(all_resin))
			var/obj/structure/necromorphs/marker/resin/iterated_resin = pick(all_resin)
			qdel(iterated_resin)
			return
	...
	if(!length(active_resin))
		...
		for(var/t in all_resin)
			var/obj/structure/necromorphs/marker/resin/iterated_resin = t
	...
	for(var/t in active_resin)
		var/obj/structure/necromorphs/marker/resin/active_resin = t
